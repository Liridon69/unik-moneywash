...
RegisterServerEvent('unik-moneywash:server:getmoney')
AddEventHandler('unik-moneywash:server:getmoney', function()
    local src = source
	local total_worth = 0
...
RegisterServerEvent('unik-moneywash:server:checkmoney')
AddEventHandler('unik-moneywash:server:checkmoney', function()
    local src = source
            ...
            if markedbills ~= nil then
                if ItemList[Player.PlayerData.items[k].name] ~= nil then 
                    if Player.PlayerData.items[k].name == "markedbills" and Player.PlayerData.items[k].amount >= 1 then 
					    ...
					    TriggerClientEvent("unik-moneywash:client:WashProggress", src)
                        break
                    else
                        TriggerClientEvent('QBCore:Notify', src, "You do not have marked money", 'error')
                        break
